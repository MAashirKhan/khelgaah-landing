...
interface PhoneScreenProps {
  kind: 'discover' | 'booking' | 'match' | 'dashboard' | 'calendar'
  accent: string
  provider: string
}

function PhoneStatusBar({ time = '9:41', dark = true }: { time?: string; dark?: boolean }) {
...
function ScreenDiscover({ accent }: { accent: string }) {
  const venues = [
...
function ScreenBooking({ accent }: { accent: string }) {
...
function ScreenDashboard({ accent, provider }: { accent: string; provider: string }) {
  return (
    <div style={{ height: '100%', background: '#0e1220', color: '#eef2ff', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' }}>
      <PhoneStatusBar />
      <div style={{ padding: '18px 20px 0' }}>
...
function ScreenMatch({ accent }: { accent: string }) {
  return (
    <div style={{ height: '100%', background: '#0e1220', color: '#eef2ff', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' }}>
      <PhoneStatusBar />
      <div style={{ padding: '18px 20px 0' }}>
...
function ScreenCalendar({ accent, provider }: { accent: string; provider: string }) {
  return (
    <div style={{ height: '100%', background: '#0e1220', color: '#eef2ff', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' }}>
      <PhoneStatusBar />
      <div style={{ padding: '18px 20px 0' }}>
        ...
        <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '32px repeat(5, 1fr)', fontSize: 9, gap: 2 }}>
          <div></div>
          {['M', 'T', 'W', 'T', 'F'].map((d, idx) => (
            <div key={idx} style={{ textAlign: 'center', color: '#7c87a3', fontWeight: 600, padding: 4 }}>{d}</div>
          ))}
...
export default function PhoneScreen({ kind, accent, provider }: PhoneScreenProps) {
