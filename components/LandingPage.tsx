...
type HeadlinePart = { text: string; s: 'serif' | 'sport' | 'serif-em' }
interface HeadlineData {
  parts: HeadlinePart[]
  sub: React.ReactNode
}

const HEADLINES: Record<'player' | 'provider', HeadlineData> = {
  player: {
    parts: [
      { text: 'Every', s: 'serif' },
      { text: 'court', s: 'sport' },
...
const ACCENT = '#4f8cff'
const PROVIDER_COLOR = '#f4c447'
const CITY = 'Karachi'

function HeadlineMix({ data, isProvider }: { data: HeadlineData; isProvider: boolean }) {
  return (
    <h1 className={`hero-h1${isProvider ? ' is-provider' : ''}`}>
      {data.parts.map((p, i) => {
        const sep = i === 0 ? '' : ' '
        if (p.s === 'sport') return <span key={i}>{sep}<span className="sport-word">{p.text}</span></span>
        if (p.s === 'serif-em') return <span key={i}>{sep}<em>{p.text}</em></span>
        return <span key={i}>{sep}{p.text}</span>
      })}
    </h1>
  )
}
...
function AudienceToggle({ value, onChange }: { value: 'player' | 'provider'; onChange: (v: 'player' | 'provider') => void }) {
  return (
...
export default function LandingPage() {
  const [audience, setAudience] = useState<'player' | 'provider'>('player')
  ...
  useEffect(() => {
    ...
    r.style.setProperty('--accent-glow', ACCENT + '66')
    r.style.setProperty('--provider', PROVIDER_COLOR)
    r.style.setProperty('--provider-glow', PROVIDER_COLOR + '55')
  }, [])
  ...
  return (
    <div className="page">
      ...
      {/* hero */}
      <section className={`col hero${audienceClass ? ' ' + audienceClass : ''}`} id="top">
        <div>
          <div className="eyebrow">
            <span className={isProvider ? 'provider' : 'accent'}>
              [ {isProvider ? 'For venues & clubs' : 'For players'} ]
            </span>
            <span>Pre-launch · {CITY} · {isProvider ? 'partner intake open' : 'founding access open'}</span>
          </div>
          <HeadlineMix data={headline} isProvider={isProvider} />
          <p className={`hero-sub${audienceClass ? ' ' + audienceClass : ''}`}>{headline.sub}</p>
          <div className={`wl-block${audienceClass ? ' ' + audienceClass : ''}`}>
            <div className="wl-label">
              {isProvider ? 'Become a founding partner' : 'Founding access · Lifetime perks'}
            </div>
            <WaitlistForm audience={audience} cta={isProvider ? 'List my venue' : 'Get founding access'} />
        ...
        <div className="hero-phones">
          <div className="phone left">
            ...
            <div className="phone-screen">
              <PhoneScreen kind={isProvider ? 'dashboard' : 'discover'} accent={ACCENT} provider={PROVIDER_COLOR} />
            </div>
          </div>
          <div className="phone center">
            <div className="phone-notch"></div>
            <div className="phone-screen">
              <PhoneScreen kind={isProvider ? 'calendar' : 'booking'} accent={ACCENT} provider={PROVIDER_COLOR} />
            </div>
          </div>
          <div className="phone right">
            <div className="phone-notch"></div>
            <div className="phone-screen">
              <PhoneScreen kind={isProvider ? 'dashboard' : 'match'} accent={ACCENT} provider={PROVIDER_COLOR} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section" style={{ borderBottom: '1px solid var(--line)' }}>
        <div className="col">
          <div className="section-head">
            <div>
              ...
              <p className="section-desc">
                Mobile-first, dark by default, fast by design. Here&rsquo;s a peek at what we&rsquo;re building.
              </p>
            </div>
          </div>
        </div>
        <div className="preview-strip">
          ...
          ] as Array<'discover' | 'booking' | 'match' | 'dashboard' | 'calendar'>).map((kind, i) => (
            <div key={i} className="preview-card">
              <div className="phone-notch"></div>
              <div className="phone-screen">
                <PhoneScreen kind={kind} accent={ACCENT} provider={PROVIDER_COLOR} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="col" id="closing">
        <div className="closing">
          <div className={`closing-left${audienceClass ? ' ' + audienceClass : ''}`}>
            ...
            <WaitlistForm audience={audience} cta={isProvider ? 'Apply as partner' : 'Claim my spot'} />
          </div>
